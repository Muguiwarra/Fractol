...
/*   Updated: 2023/03/15 06:25:34 by nabboune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_alloc(t_global *mlx)
{
	...
	if (!mlx->range)
		return (free(mlx->image), free(mlx->zn), free(mlx->complex), free(mlx), 0);
	mlx->screen = (t_pixel *) malloc(sizeof(t_pixel));
	if (!mlx->screen)
		return (free(mlx->image), free(mlx->zn), free(mlx->complex), free(mlx->range), free(mlx), 0);
	mlx->pos = (t_mouse_pos *) malloc(sizeof(t_mouse_pos));
	if (!mlx->pos)
		return (free(mlx->image), free(mlx->zn), free(mlx->complex), free(mlx->range), free(mlx->screen), free(mlx), 0);
	return (1);
}
